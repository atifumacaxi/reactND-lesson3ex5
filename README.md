<<<<<<< 9fe9dbfaa70258f6aee1b065bb4d916c16c073d3
...
An array is not created with `.push()`, so it will works only after an array is "started". For example:

`this.usersByMovie[movieID].push(profile.userID)` will work only after `this.usersByMovie[movieID] = [profile.userID]`.

### Git

I created my very first `.gitignore` file and saw how it works. The main point is: after you edit/create a `.gitignore` file, you should refresh/resync what Git is considering to your `commit` or `add` command.

So, running `git rm -r --cached`, git will remove (rm) all files cached in the command (`-r` is for recursive, i.e., removes all folders and files inside all folders). After this, my `git add .` and `git status` showed me exaclty what I expected.

Almost every code here belongs to Udacity - excellent course - Credits to Udacity.
=======
